...
function get_promos(){
    ...
    $.ajax({
           ...
           $.each(screen_promos.promos,function(idx,val){
                  ...
                  var button = '<button id="boton'+val.id+'" class="share_promo" onclick=" participar('+val.id+',\''+val.content+'\');">Participar</button>';
                  if(val.estoy_participando=="true"){
	                	button = '<button id="boton'+val.id+'" style="width: 43%; float: left; margin-left: 5%;" class="share_promo" onclick=" no_participar('+val.id+',\''+val.content+'\');">Dejar de participar</button><button id="boton_share'+val.id+'"style="width: 40%; float: right; margin-right: 5%;" class="share_promo" onclick="share_promo(\''+val.content+'\');">Compartir</button>';
                  }
...
screen_config.show = function(){
    ...
    screen_timeline.css('pointer-events','none');
    
};
screen_config.hide = function(){
    focus_trivia = true; screen_config.addClass('lefted');
    screen_timeline.css('pointer-events','all');
...
function cambiar_foto(){
    ...
    if(isNaN(user_login)){
        navigator.notification.confirm(
                                       'Deseas actualizar tu foto, ¿de dónde la tomamos?',
                                       function(btn){
                                       if(btn==1){
                                       useCameraProfile();
                                       }else if(btn==2){
...
register: function(){
    ...
    if(tel.length<8){
        navigator.notification.alert('Tu número de teléfono no está completo',null,'Voz de Dios','Ok');
        return false;
    }else if(tel.length>8){
        navigator.notification.alert('Has ingresado un número de teléfono muy largo',null,'Voz de Dios','Ok');
        return false;
...
function register(){
    ...
    var nacimiento_escogido = document.getElementById("fecha_nacimiento").value;
    console.log(nacimiento_escogido);
    nacimiento_escogido = nacimiento_escogido.split('-');
    if(nacimiento_escogido[0]>this_year){
        navigator.notification.alert('El año no puede ser mayor al actual', null, 'Voz de Dios','Ok');
        return false;
    }
    
    new_user_birthday = new Date(nacimiento_escogido[0],nacimiento_escogido[1],nacimiento_escogido[2]);
    new_user_gender =  $("#genero_register").val();
    
    console.log(new_user_country);
    if(new_user_pass == "" || new_user_name == "" || new_user_login==""){
        navigator.notification.alert('Debes definir una contraseña, tu correo y un nombre público.', null, 'Voz de Dios','Ok');
        return false;
    }
    $.ajax({
           url: urlws,
